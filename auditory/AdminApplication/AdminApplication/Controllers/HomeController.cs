...
namespace AdminApplication.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> ImportUsers(IFormFile file)
        {
            ...
            string URL = "http://localhost:5033/api/admin/ImportUsers";
        ...
        public async Task<IActionResult> ExportOrders()
        {
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                var client = new HttpClient();
                string URL = "http://localhost:5033/api/admin/getallorders";
                ...
                for (int i = 1; i <= result.Count; i++)
                {
                    ...
                    for (int j = 0; j < currentOrder.ProductsInOrders.Count(); j++)
                    {
                        var currentProductInOrder = currentOrder.ProductsInOrders.ToList().ElementAt(j);

                        worksheet.Cell(1, j + 3).Value = "Product-" + (j + 1);
                        worksheet.Cell(i + 1, j + 3).Value = currentProductInOrder.Product.Name;
                    }
                }
        ...
        public async Task<IActionResult> CreateInvoice(Guid id)
        {
            ...
            string URL = $"http://localhost:5033/api/admin/GetOrderDetails?id={id}";
            ...
            foreach (var item in result.ProductsInOrders)
            {
                totalPrice += item.Quantity * item.Product.Price;
                sb.AppendLine(item.Product.Name + " with quantity of: " + item.Quantity + " and price of: " + item.Product.Price + "$");
            }
